...
def mutate_tensor(
    ...
    size=indices.size()[0]
    for i in range(size):
            x[int(indices[i][0])][int(indices[i][1])]=values[i]
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return x
...
def slice_assignment_practice(x: Tensor) -> Tensor:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    x[0:6,0:6]=torch.Tensor([[0,1,2,2,2,2],[0,1,2,2,2,2],[3,4,3,4,5,5],[3,4,3,4,5,5]])
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return x
...
def make_one_hot(x: List[int]) -> Tensor:
    ...
    y.scatter_(1,x.unsqueeze_(1),1.0)
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return y
...
def zero_row_min(x: Tensor) -> Tensor:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    value,index=torch.min(x,dim=1,keepdim=True)
    y=x.clone()
    torch_index=torch.zeros_like(x,dtype=torch.bool)
    torch_index.scatter_(1,index,True)
    y[torch_index]=0
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
...
def normalize_columns(x: Tensor) -> Tensor:
    ...
    M,N=x.shape
    mu=x.sum(dim=0)/M
    std = torch.sqrt((x * x).sum(dim=0)/ M - mu * mu)
    std[x==0]=1
    y=(x-mu)/std
    ##########################################################################
    #                            END OF YOUR CODE                            #
...
def challenge_mean_tensors(xs: List[Tensor], ls: Tensor) -> Tensor:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    index=torch.arange(len(ls),device=ls.device)
    index_repeated=torch.repeat_interleave(index,ls)
    sums=torch.bincount(index_repeated,weights=torch.cat(xs))
    y=sums.to(torch.float64)/ls.to(torch.float64)    
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return y
...
def challenge_get_uniques(x: torch.Tensor) -> Tuple[Tensor, Tensor]:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    values,index=torch.sort(x)
    mask=torch.cat([torch.tensor([True],dtype=torch.bool),values[1:]!=values[:-1]])
    uniques=values[mask]
    indices=index[mask]
    ##########################################################################
    #                            END OF YOUR CODE                            #
