...
def compute_distances_two_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    trainset=x_train.view(num_train,-1)
    testset=x_test.view(num_test,-1)
    for i in range(num_test):
        for j in range(num_train):
            diff=trainset[i]-testset[j]
            dists[j,i]=torch.sum(diff*diff)
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def compute_distances_no_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    trainset=x_train.view(num_train,-1)
    testset=x_test.view(num_test,-1)
    squatrain=(trainset*trainset).sum(dim=1).unsqueeze_(1)
    squatest=(testset*testset).sum(dim=1).unsqueeze_(0)
    cross=trainset @ (testset.t())
    dists=squatrain+squatest-cross*2
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def predict_labels(dists: torch.Tensor, y_train: torch.Tensor, k: int = 1):
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    values1,indices=torch.sort(dists,dim=0)[0:k]
    values2,y_pred=torch.sort(torch.bincount(y_train,weights=indices))[0]
    y_pred.squeeze(1)
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
...
class KnnClassifier:
    ...
    def predict(self, x_test: torch.Tensor, k: int = 1):
        ...
        trainset=self.x_train.view(num_train,-1)
        squa_test=torch.sum(testset*testset,dim=1).unsqueeze_(0)
        squa_train=torch.sum(trainset*trainset,dim=1).unsqueeze_(1)
        cross=trainset @ testset.t()
        dists=squa_test+squa_train-cross*2
        values,indices=torch.sort(dists,dim=0)[0:k]
        values1,y_test_pred=torch.sort(torch.bincount(self.y_train))[0]
        y_test_pred.squeeze_(1)
        ######################################################################
        #                         END OF YOUR CODE                           #
        ######################################################################
        return y_test_pred
